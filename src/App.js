...
function MyTimer({ expiryTimestamp, interval }) {
  ...
  return (
    <div className="flex flex-col  items-center justify-center h-screen w-screen cursor-pointer" onClick={startTimer}>
...
export default function App() {
  ...
  const setUserValue = (val) => {
    ...
    const newOld = [...new Set([...oldItems, val].sort((function(a, b){return a-b})))]
    setOldItems(newOld);
    setState(val);
  }
  ...
  return (
    <div className="w-screen h-screen overflow-hidden bg-black text-white flex items-center justify-center">
      { state ? <MyTimer expiryTimestamp={initTime} interval={state} /> : (<div className="flex flex-col">
        <div className="flex ">
          <input value={itm} onChange={event => setItm(parseInt(event.target.value))} type="number" className="px-3 text-black rounded-lg w-20 mr-5"/>
          <button className="text-black bg-blue-300 text-blue-800 rounded-lg py-2 px-4" onClick={() => { setUserValue(itm)}}>Commencer !</button>
        </div>
        <div className="flex mt-8 space-x-5">
          {oldItems.map(it => {
            return <button key={it} className="bg-orange-300 text-orange-800 rounded-lg py-2 px-4"
                            onClick={() => setState(it)}>{it}</button>;
          })}
        </div>
      </div>)
      }
